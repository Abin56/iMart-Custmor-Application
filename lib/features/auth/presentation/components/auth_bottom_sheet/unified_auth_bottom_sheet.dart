...
class _UnifiedAuthBottomSheetState
  ...
  void initState() {
    ...
    for (final controller in _mobileOtpController.otpControllers) {
      controller.addListener(_onOtpChanged);
    }
    for (final controller in _forgotPasswordController.otpControllers) {
  ...
  void dispose() {
    ...
    _emailLoginController.passwordController.removeListener(
      _onLoginFieldChanged,
    );
        ...
        child: SingleChildScrollView(
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [_buildCurrentScreen(authState)],
            ),
          ),
        ),
      ),
  ...
  void _handleAuthStateChange(AuthState state) {
    AuthStateHandler.handleAuthStateChange(
      ...
      mobileOtpController: _currentMode == AuthMode.mobileOTP
          ? _mobileOtpController
          : null,
      emailLoginController: _currentMode == AuthMode.emailPassword
          ? _emailLoginController
          : null,
      forgotPasswordController: _currentMode == AuthMode.forgotPassword
      ...
      setError:
          (
            ...
            setState(() {
              if (otpError != null &&
                  _mobileOtpController.hasOtpError != hasError) {
                _mobileOtpController.setOtpError(otpError);
              }
              if (mobileError != null &&
                  _mobileOtpController.hasMobileError != hasError) {
                _mobileOtpController.setMobileError(mobileError);
              }
              if (loginError != null &&
                  _emailLoginController.hasLoginError != hasError) {
                _emailLoginController.setLoginError(loginError);
              }
              if (forgotOtpError != null &&
                  _forgotPasswordController.hasOtpError != hasError) {
