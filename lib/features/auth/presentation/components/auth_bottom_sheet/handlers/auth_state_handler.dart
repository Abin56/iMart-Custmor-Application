...
class AuthStateHandler {
  static void handleAuthStateChange({
    ...
    required Function(bool, {String? otpError, String? mobileError, String? loginError, String? forgotOtpError}) setError,
  }) {
    if (state is OtpSent) {
      if (mobileOtpController != null) {
        mobileOtpController.showOtpField = true;
        mobileOtpController.clearOtp();
        setSubmitting(false);
    ...
    if (state is AuthError) {
      ...
      if (currentMode == AuthMode.mobileOTP && mobileOtpController != null) {
        if (mobileOtpController.showOtpField && isOtpError) {
          ...
          setError(true, otpError: ErrorMessageHandler.getOtpErrorMessage(failure));
        } else if (!mobileOtpController.showOtpField) {
          // Show mobile number error inline before OTP field is shown
          setError(true, mobileError: ErrorMessageHandler.getUserFriendlyError(failure));
        } else {
          AppSnackbar.error(context, ErrorMessageHandler.getUserFriendlyError(failure));
        }
      } else if (currentMode == AuthMode.emailPassword && emailLoginController != null) {
        // Show login error inline for email/password mode
        setError(true, loginError: ErrorMessageHandler.getUserFriendlyError(failure));
      } else if (currentMode == AuthMode.forgotPassword && forgotPasswordController != null) {
        if (forgotPasswordController.showOtpField && isOtpError) {
          setError(true, forgotOtpError: ErrorMessageHandler.getOtpErrorMessage(failure));
        } else {
          AppSnackbar.error(context, ErrorMessageHandler.getUserFriendlyError(failure));
        }
      } else {
